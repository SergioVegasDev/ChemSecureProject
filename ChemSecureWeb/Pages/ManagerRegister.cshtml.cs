...
namespace ChemSecureWeb.Pages
{
    public class ManagerRegisterModel : PageModel
    {
        ...
        public ManagerRegisterModel(IHttpClientFactory httpClientFactory, ILogger<RegisterModel> logger, IConfiguration configuration)
        {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            var response = await httpClient.PostAsync("api/Auth/manager/register", content);

            // **Handle responses**
            if (response.IsSuccessStatusCode)
            {
                _logger.LogInformation("Manager register successful.");
